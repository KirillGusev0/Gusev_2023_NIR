...
def convert_data(input_csv, output_csv):
    ...
    df = pd.read_csv(r"C:\Users\kirdr\.spyder-py3\TimeDataSets\Daily-train.csv")
    ...
    for i, label in enumerate(labels):
        ...
        for j in range(1, len(df.columns)):  # Начинаем с 1, чтобы пропустить первый столбец с метками
            ...
            if not pd.isna(data):  # Проверяем, что данные не являются пустыми
                reshaped_data.append([label, current_time.strftime('%Y-%m-%d'), data])
            current_time += timedelta(days=1)  # Увеличение временной метки на один час
...
input_file = r"C:\Users\kirdr\.spyder-py3\TimeDataSets\Daily-train.csv"
output_file = r"C:\Users\kirdr\.spyder-py3\TimeDataSets\Daily-train-processed.csv"
